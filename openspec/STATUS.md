...
and the two backlogs were **consolidated into `docs/BACKLOG.md`** (PR #54). **M5 Event Infrastructure is now
being implemented** on branch `phase3-event-infrastructure`: `EDR-EVENTBUS-01` (D1–D11) +
`phase3-event-infrastructure` (**9/43 — Groups 1–2 done**, 10 groups). See `docs/engineering/PHASE3-STATUS.md`.)_
...
| **phase3-event-infrastructure** (M5) | **Implementing — 9/43 tasks (Groups 1–2 done)** (branch `phase3-event-infrastructure`, started 2026-07-27) | 2026-07-28 | `EDR-EVENTBUS-01` (D1–D11), the platform-owned event bus. 10 groups EB-01…EB-11: `internal/platform/eventbus` + a `bus` database, full `Envelope` threading, integration-contract v1 (schema-guarded), `Publisher`, stream `Reader`/drain (gap-free + D8 stream-halt), consumer inbox (exactly-once **application**), stream + interest-set subscriptions, `cmd/knowledge` + in-process runner, black-box SBOM→OpenVEX e2e + focused D5/D6/D8 tests, docs. **Done:** Group 1 (EB-01 scaffold + `bus` DB), Group 2 (EB-02 — `Envelope` threaded through all four outboxes + both consumers), both gated `make check` green (2026-07-28). **Next:** Group 3 (EB-03 schema guard). **Started 2026-07-27** (`/opsx:apply`) | — |
