...
| **B3** | Red Hat VEX / CSAF applicability | full subsystem — fetch client + per-EL-stream verdict + epoch rpm compare + scheduler | **client + verdict engine absent**; only a `redhatACL` translator behind the unwired `feed.Registry` | HIGH | ⚠️ (§C implied "just wiring") |
