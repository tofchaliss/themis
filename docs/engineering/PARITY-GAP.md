...
| **B3** | Red Hat VEX / CSAF applicability | full subsystem — fetch client + per-EL-stream verdict + epoch rpm compare + scheduler | **fetch client + scheduler DONE 2026-08-01** (`RedHatClient` per-CVE Hydra fetch → vendor vuln-facts + `not_affected` applicability, relevance-bounded, `THEMIS_REDHAT_ENABLED`; precedence ranks `redhat` distro-authoritative; covers RHEL/Rocky/Alma). **Still open (PR3):** the per-EL-stream + epoch-rpm **fixed-verdict** engine (`affected_release` NEVRA compare) — this feed emits the vendor's explicit `not_affected` only | HIGH→MED | ✅ feed / ⏳ verdict |
| **B4** | Generic vendor-VEX feed crawler (CSAF dir / zip / apk) | `adapter/vexfeed/*` + scheduler | **no fetcher/crawler**; only a `vexACL` translator behind the unwired Registry | MED-HIGH | ⚠️ |
