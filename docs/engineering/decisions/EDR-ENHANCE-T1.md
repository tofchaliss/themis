...
| **GUI-4** | Per-distro feed-health rows (`osv/alpine`, `osv/rocky`, …) | `GET /feeds` collapses all distros into one row; a quiet distro is invisible |
...
3. **GUI-4 tier semantics**: per-distro rows stay informational tier (a quiet distro never reads as
   degraded) — carried over from EDR-KNOWLEDGE-01; no new decision, just confirmation.

## Delivery order

GUI-12 → KN-SCAN-3 → GUI-4 → GUI-10 → docs paragraph. Each lands as its own branch + gate; no
