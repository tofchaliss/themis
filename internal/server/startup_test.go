...
func TestBootSuccess(t *testing.T) {
	...
	cfg := bootConfig{
		...
		hooks: bootHooks{
			connect: func(context.Context, string, int32) (databasePool, error) {
				return stubPool{}, nil
			},
...
func TestBootVerifySchemaFailure(t *testing.T) {
	...
	cfg := bootConfig{
		...
		hooks: bootHooks{
			...
			runMigrations: func(string, string) error { return nil },
			verifySchemaVersion: func(string, string) error {
				return errors.New("schema verify failed")
			},
		},
	}
	_, err := bootWithConfig(context.Background(), zap.NewNop(), cfg)
...
func TestBootOptions(t *testing.T) {
	...
	WithWorkerPool(NoopWorkerPool{})(&cfg)
