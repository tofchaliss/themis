...
func (r *Relay) DeliverPending(ctx context.Context) (int, error) {
	rows, err := r.pool.Query(ctx, `
		SELECT id, finding_id, event_type, payload, occurred_at
	...
	for rows.Next() {
		var n OutboxNote
		if err := rows.Scan(&n.ID, &n.FindingID, &n.EventType, &n.Payload, &n.OccurredAt); err != nil {
			rows.Close()
			return 0, err
		}
	...
	for _, n := range notes {
		if err := r.pub.Publish(ctx, n); err != nil {
			if _, uerr := r.pool.Exec(ctx, `UPDATE governance_outbox SET attempts = attempts + 1 WHERE id = $1`, n.ID); uerr != nil {
				return delivered, uerr
			}
			continue
		}
		if _, err := r.pool.Exec(ctx, `UPDATE governance_outbox SET sent_at = now() WHERE id = $1`, n.ID); err != nil {
