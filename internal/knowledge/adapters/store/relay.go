...
// Relay delivers pending Knowledge outbox rows exactly-once-eventually (BCK-0041): it
// reads each un-sent row as a full Envelope, publishes it, and marks it sent on success
// or bumps its attempt count on failure so it is retried on the next pass.
type Relay struct {
	pool  *pgxpool.Pool
	pub   Publisher
	batch int
...
// NewRelay builds a Relay delivering up to batch envelopes per pass.
...
func (r *Relay) DeliverPending(ctx context.Context) (int, error) {
	...
	for _, env := range envs {
		if err := r.pub.Publish(ctx, env); err != nil {
			if _, uerr := r.pool.Exec(ctx, `UPDATE knowledge_outbox SET attempts = attempts + 1 WHERE id = $1`, env.ID); uerr != nil {
				return delivered, uerr
			}
			continue
		}
		if _, err := r.pool.Exec(ctx, `UPDATE knowledge_outbox SET sent_at = now() WHERE id = $1`, env.ID); err != nil {
